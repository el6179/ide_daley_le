...
void forward(float speedL, float speedR, float shift, float posErr){
	...
	float x = fabs(posErr); // fPosErrs
	
	float multiplier = (0.00458333*x*x) + (0.00125*x) + 0.02916667;

	float speedErr = posErr * multiplier;
	
	PID(speedL, el, &VLact, &pwml);
	PID(speedR, er, &VRact, &pwmr);
...
int main(void) {
	...
	float rightMotorSpeed, leftMotorSpeed, center, shift, posErr, speed;
	float const baseSpeed = 30;
	...
	for(;;){
      ...
      if (pch == 0x20){
       ...
       shift = 2.5;
		...
		// Changing motor speed - screws up the PID 
