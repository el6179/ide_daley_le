...
void forward(float speedL, float speedR, float posErr){
	
  float speedErr = (kp * posErr) + (ki * (posErr + lastPosErr)/2) + (kd * (posErr - lastPosErr));
	lastPosErr = posErr;
	
  speedR = speedR + speedErr;
	speedL = speedL - speedErr;
...
void stop(void){/*Stop Car*/
	forward(0.0, 0.0, 0);/*backward(0.0) should produce same result*/
	//Disable Motor
	P3->OUT &= ~BIT6;
	P3->OUT &= ~BIT7;
}
...
int main(void) {
	...
	float rightMotorSpeed, leftMotorSpeed, center, shift, posErr;
	float baseSpeed = 27;
	...
	for(;;){
      ...
      if (pch == 0x20){
       ...
       shift = 2.5;
		...
		if (shift < 5.25 && shift > 4.75) {
			baseSpeed = 40;
      ki = 0.6;
		} 
		else if (shift < 5.4 && shift > 4.6){
			baseSpeed = 30;
      ki = 0.0;
		}
		else {
	  ...
	  //uart2_put(str);
		...
		forward(baseSpeed, baseSpeed, posErr);
